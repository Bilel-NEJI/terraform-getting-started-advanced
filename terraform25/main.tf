...
# Step 29 | task 2:
# here we come to add the "size" argument and define when we comment the "size" variable (under server/variables.tf) = meaning that by commenting it, it returns to
module "server" {
  source          = "./terraform24/modules/server"
  ami             = data.aws_ami.ubuntu.id
  # this is the added argument when we remove/comment the "default" argument 
  size            = "t2.micro"
...
# is this the suite of returning another argument that we already added in the "outputs.tf" under "server" module
# --> "apply"
output "size" {
  value = module.server.public_dns
}
