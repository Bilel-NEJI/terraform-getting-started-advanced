...
# Step 47 | task 1: Use basic numerical functions to select data
# we add new locals block just after adding the 03 new variables
# the local called "maximum" will choose the maximum number of the 03 entered variables
# the local called "minimum" will choose the minimum number of the 03 entered variables with 02 other random numbers that we provided/entered
...
# Step 47 | task 2: Manipulate strings using Terraform functions
# the current state before modifying is; the vpc tags are:
# the "vpc-name" is taken from the "variables.tf" file which equal to "demo_vpc"
# and the "vpc-environment" is equal to "var.environment" = "dev"
# we'll modify this resource a little bit; modifying these two tags
# then "apply" --> the changes will be in our tags
#Define the VPC 
resource "aws_vpc" "vpc" {
  ...
  tags = {
    ...
    # Terraform   = "true"
    # after modifying; we want to display them always in Upper case
    # also we can make the tag terraform shown is upper cases
...
# Step 47 | task 1: Use basic numerical functions to select data
# so after creating the 02 locals "maximum" and "minimum" we come here add 02 outputs to display and see them
# then "apply" --> it works fine
output "max_value" {
  value = local.maximum
}
